...
export default function RelayMotionDemo() {
  ...
  useGSAP(
    () => {
      ...
      tl.to(
        '.req-node',
        { x: 260, opacity: 0, duration: 1, stagger: 0.1, ease: 'power2.in' },
        'scene1+=2.5'
      );
      ...
      tl.to(
        '.route-out-req.r1',
        { x: 300, y: -120, duration: 1.0, ease: 'power2.inOut' },
        'scene3+=0.5'
      );
      ...
      tl.to(
        '.topology-view',
        { scale: 2.2, x: 50, y: 0, duration: 1.8, ease: 'power3.inOut' },
        'scene4'
      );
      ...
      tl.to(
        '.auth-check.c1 .status-text',
        { textContent: 'verified ✓', color: '#34d399', duration: 0.1 },
        'scene4+=2.1'
      );

      tl.to('.auth-check.c2', { opacity: 1, x: 0, duration: 0.4 }, 'scene4+=2.4');
      tl.to(
        '.auth-check.c2 .status-text',
        { textContent: 'Production ✓', color: '#34d399', duration: 0.1 },
        'scene4+=2.7'
      );

      tl.to('.auth-check.c3', { opacity: 1, x: 0, duration: 0.4 }, 'scene4+=3.0');
      tl.to(
        '.auth-check.c3 .status-text',
        { textContent: '184 / 1000 ✓', color: '#34d399', duration: 0.1 },
        'scene4+=3.3'
      );
      ...
      tl.to(
        '.topology-view',
        { opacity: 0, scale: 4, duration: 1.2, ease: 'power3.in' },
        'scene5+=0.2'
      );
      ...
      tl.to(
        '.metric-val.reqs',
        { textContent: '48.2', roundProps: 'textContent', duration: 1.5, ease: 'power2.out' },
        'scene5+=2.2'
      );
      tl.to(
        '.metric-val.succ',
        { textContent: '99.8', roundProps: 'textContent', duration: 1.5, ease: 'power2.out' },
        'scene5+=2.2'
      );
      tl.to(
        '.metric-val.lat',
        { textContent: '42', roundProps: 'textContent', duration: 1.5, ease: 'power2.out' },
        'scene5+=2.2'
      );
      tl.to(
        '.metric-val.keys',
        { textContent: '11', roundProps: 'textContent', duration: 1.5, ease: 'power2.out' },
        'scene5+=2.2'
      );
      ...
      tl.to(
        '.topology-view',
        { opacity: 1, scale: 0.75, x: 0, y: 0, duration: 1.5, ease: 'power3.inOut' },
        'scene6+=0.8'
      );
      ...
      tl.to(
        '.continuous-flow-req.r1',
        { x: 700, duration: 2.5, ease: 'none', repeat: 1 },
        'scene6+=2.0'
      );

      tl.to('.continuous-flow-req.r2', { opacity: 1, duration: 0.2 }, 'scene6+=2.5');
      tl.to(
        '.continuous-flow-req.r2',
        { x: 700, duration: 2.5, ease: 'none', repeat: 1 },
        'scene6+=2.5'
      );
      ...
      tl.to(
        '.topology-view',
        { opacity: 0, scale: 0.6, duration: 1.2, ease: 'power2.in' },
        'scene7'
      );
      tl.set('.topology-view', { display: 'none' });
    ...
    return (
      <div className="w-full h-full min-h-[600px] flex items-center justify-center bg-[#050505] text-[#ededed] font-sans">
        <div className="text-center">
          ...
          <p className="text-xl text-[#a1a1aa] mb-8 font-medium">
            API infrastructure for teams shipping fast.
          </p>
          <button className="px-6 py-3 bg-white text-black rounded-lg font-semibold shadow-lg">
            Get started &rarr;
          </button>
        </div>
      </div>
    ...
    >
      ...
      <div className="topology-view">
        ...
        <div className="auth-panel absolute z-50 w-80 premium-panel rounded-xl p-6">
          <div className="text-xs font-mono text-[#a1a1aa] mb-5 tracking-[0.15em] border-b border-white/10 pb-3">
            SECURITY LAYER
          </div>
      ...
      <div className="dashboard-view">
        ...
        <div className="h-14 border-b border-white/10 flex items-center px-8 justify-between bg-[#050505]">
          <div className="flex items-center gap-6">
            ...
            <div className="px-3 py-1 rounded bg-[#111] text-xs font-mono text-[#a1a1aa] border border-white/10">
              production
            </div>
          </div>
          ...
          <div className="flex gap-6">
            <div className="flex-1 bg-[#111] premium-border rounded-xl p-5 shadow-md">
              ...
              <div className="text-4xl font-mono text-white flex items-baseline gap-1">
                <span className="metric-val reqs">0.0</span>
                <span className="text-xl text-[#71717a]">K</span>
              </div>
            </div>
            <div className="flex-1 bg-[#111] premium-border rounded-xl p-5 shadow-md">
              <div className="text-xs text-[#a1a1aa] font-mono mb-2 tracking-wide">
                SUCCESS RATE
              </div>
              <div className="text-4xl font-mono text-[#34d399] flex items-baseline gap-1">
                <span className="metric-val succ">0.0</span>
            ...
            <div className="flex-1 bg-[#111] premium-border rounded-xl p-5 shadow-md">
              ...
              <div className="text-4xl font-mono text-white flex items-baseline gap-1">
                <span className="metric-val lat">0</span>
                <span className="text-xl text-[#71717a]">ms</span>
              </div>
            </div>
            <div className="flex-1 bg-[#111] premium-border rounded-xl p-5 shadow-md">
              <div className="text-xs text-[#a1a1aa] font-mono mb-2 tracking-wide">ACTIVE KEYS</div>
              <div className="text-4xl font-mono text-white">
                <span className="metric-val keys">0</span>
              </div>
            </div>
          </div>
              ...
              <div className="log-row-anim rounded-lg bg-[#1a1a1a] border border-white/10 overflow-hidden shadow-md">
                ...
                <div className="log-expanded px-6 bg-[#1a1a1a] flex gap-12 border-t border-white/10">
                  <div className="flex-1">
                    <div className="text-xs text-[#71717a] font-mono mb-3 tracking-widest">
                      RESPONSE BODY
                    </div>
                  ...
                  <div className="w-56">
                    ...
                    <div className="flex justify-between text-sm font-mono mb-2 text-[#ededed]">
                      <span>Region</span>
                      <span className="text-[#a1a1aa]">iad1</span>
                    </div>
                    <div className="flex justify-between text-sm font-mono mb-2 text-[#ededed]">
                      <span>Cache</span>
                      <span className="text-[#a1a1aa]">MISS</span>
                    </div>
                    <div className="flex justify-between text-sm font-mono text-[#ededed]">
                      <span>Key</span>
                      <span className="text-[#a1a1aa]">sk_live_...</span>
                    </div>
                  </div>
                </div>
              </div>
      ...
      <div className="brand-frame absolute inset-0 flex items-center justify-center bg-[#000000] z-50">
        <div className="text-center">
          <h1 className="brand-logo text-7xl font-bold mb-6 font-pixel tracking-widest text-white drop-shadow-[0_0_20px_rgba(255,255,255,0.2)]">
            RELAY_
          </h1>
          <p className="brand-subtitle text-2xl text-[#a1a1aa] mb-10 font-medium">
            API infrastructure for teams shipping fast.
          </p>

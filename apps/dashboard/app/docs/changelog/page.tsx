    ...
    changes: [
      {
        type: 'ADDED',
        area: 'API',
        text: 'Added support for IP-based and header-based rate limiting rules.',
      },
      {
        type: 'ADDED',
        area: 'API',
      ...
      },
      {
        type: 'FIXED',
        area: 'Dashboard',
        text: 'Fixed an issue where the dashboard would show healthy APIs as degraded under certain edge cases.',
      },
    ],
  },
  {
    ...
    changes: [
      {
        ...
        text: (
          <>
            All outbound webhooks now include an{' '}
            <code className="docs-inline">x-relay-signature</code> header using HMAC SHA-256.
          </>
        ),
      },
      {
        type: 'IMPROVED',
        area: 'Core',
        text: 'Failed webhooks now automatically retry with exponential backoff up to 5 times over 3 hours.',
      },
      {
        type: 'BREAKING',
        area: 'API',
        text: 'Legacy un-signed webhook endpoints will be deprecated in v3.0.',
      },
    ],
  },
  {
    ...
    changes: [
      { type: 'ADDED', area: 'Core', text: 'Launched the Relay Gateway and Dashboard.' },
      {
        type: 'ADDED',
        area: 'API',
        text: 'Support for dynamic routing, basic API key authentication, and traffic observability.',
      },
    ],
  },
];
  ...
  return (
    <main className="docs-main changelog-page">
      ...
      <div className="changelog-header-row">
        <h1 className="docs-h1" style={{ marginBottom: 0 }}>
          Changelog
        </h1>
      </div>
        ...
        {filteredEntries.map((entry, idx) => (
          <div key={idx} className="changelog-entry">
            ...
            <div className="changelog-dot-container">
              <div
                className={`changelog-dot ${idx === 0 && activeFilter === 'All' ? 'active' : ''}`}
              ></div>
              ...
              <div className="changelog-changes">
                {entry.changes.map((change, cidx) => (
                  <div key={cidx} className="change-line">
                    <span className={`change-tag ${change.type.toLowerCase()}`}>{change.type}</span>

  ...
  const proxyCode = [
    {
      ...
      code: (
        <>
          ...
          app.set(<span className="docs-tok-str">'trust proxy'</span>, <span className="docs-tok-key">true</span>);
        </>
      ),
    },
  ];
  ...
  const middlewareCode = [
    {
      ...
      code: (
        <>
          ...
          {'  '}<span className="docs-tok-key">const</span> clientIp = req.headers[<span className="docs-tok-str">'x-relay-client-ip'</span>];{'\n'}
          {'  '}<span className="docs-tok-key">const</span> forwardedFor = req.headers[<span className="docs-tok-str">'x-forwarded-for'</span>];{'\n\n'}
          {'  '}<span className="docs-tok-key">if</span> (!clientIp && !forwardedFor) {'{\n'}
          {'    '}<span className="docs-tok-key">return</span> res.status(403).json({'{'} error: <span className="docs-tok-str">'Direct access forbidden'</span> {'}'});{'\n'}
          {'  }\n'}
          {'  '}next();{'\n'}
          {'}'};{'\n\n'}
          app.use(relayOnly);
  ...
  return (
    <>
      <main className="docs-main">
        ...
        <p className="docs-p">
          Relay automatically appends the original client IP to the <code className="docs-inline">x-forwarded-for</code> header and explicitly sets the <code className="docs-inline">x-relay-client-ip</code> header. You can use these headers to identify the real origin of the request.
        </p>
        
        <ul className="docs-ul" style={{ paddingLeft: '20px', marginBottom: '14px', fontSize: '15px', color: 'var(--text)', lineHeight: '1.65' }}>
          <li style={{ marginBottom: '6px' }}>
            <strong>x-forwarded-for:</strong> Standard header showing the originating IP address.
          </li>
          <li style={{ marginBottom: '6px' }}>
            <strong>x-relay-client-ip:</strong> Relay-specific header for reliable IP identification.
          </li>
        </ul>
